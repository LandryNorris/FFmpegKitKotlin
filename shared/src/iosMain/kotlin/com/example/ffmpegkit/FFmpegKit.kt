...
import cocoapods.ffmpegkit.FFmpegKit as FFmpeg
...
actual object FFmpegKit {
    actual fun executeWithArgumentsBlocking(arguments: List<String>): FFmpegSession =
        FFmpeg.executeWithArguments(arguments)!!.toShared()

    actual suspend fun executeWithArguments(arguments: List<String>): FFmpegSession =
        suspendCoroutine { cont ->
            FFmpeg.executeWithArgumentsAsync(arguments) {
    ...
    actual fun listSessions(): List<FFmpegSession> = FFmpeg.listSessions()
        ?.filterIsInstance<cocoapods.ffmpegkit.FFmpegSession>()?.map { it.toShared() } ?: listOf()
}